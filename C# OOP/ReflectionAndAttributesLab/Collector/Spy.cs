...
namespace Stealer
{
    public class Spy
    {
        ...
        {
            MethodInfo[] getters = StealMethodInfo(className).Where(x => x.Name.StartsWith("get")).ToArray();
            MethodInfo[] setters = StealMethodInfo(className).Where(x => x.Name.StartsWith("set")).ToArray();
